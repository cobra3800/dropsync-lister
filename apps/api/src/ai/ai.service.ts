...
try {
  ...
  const generatedAspects: Record<string, string[]> = {};
  ...
  for (const aspectName of requiredAspects) {
    ...
    if (
      ...
      generatedAspects[aspectName] = [value[0].trim()];
    } else if (typeof value === 'string' && value.trim()) {
      generatedAspects[aspectName] = [value.trim()];
    } else {
      generatedAspects[aspectName] = ['Does Not Apply'];
    }
  }
